...
class DashboardController extends Controller
{
    ...
    public function getDashboardData()
    {
        try {
            ...
            // Generar fechas y rellenar con datos (modificado para corregir el orden)
            for ($i = $daysToShow - 1; $i >= 0; $i--) {
                $date = now()->subDays($i);
                $fechas[] = $date->translatedFormat('j M');

                $dateKey = $date->format('Y-m-d');
                $total = isset($salesData[$dateKey]) ? $salesData[$dateKey] : 0;
                $totales[] = number_format((float)$total, 2);
            }
